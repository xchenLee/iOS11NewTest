...
class CoreMLController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    @IBAction func imageTapped(_ sender: UITapGestureRecognizer) {
        self.inputImage.layer.sublayers?.removeAll()
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [String : Any]) {
        ...
        DispatchQueue.global().async {
            ...
            self.highlightFaces(for: image, complete: { (resultImage) in
    ...
    func detecFace(_ image: UIImage) {
    ...
    func addShapesToFace(forObservations observations: [VNFaceObservation]) {
        let layers: [CAShapeLayer] = observations.map { observation in
            ...
            let w = observation.boundingBox.size.width * self.inputImage.bounds.width
            let h = observation.boundingBox.size.height * self.inputImage.bounds.height
            let x = observation.boundingBox.origin.x * self.inputImage.bounds.width
            let y = abs(((observation.boundingBox.origin.y * (self.inputImage.bounds.height)) - self.inputImage.bounds.height)) - h
        ...
        for layer in layers {
            self.inputImage.layer.addSublayer(layer)
        }
    }
